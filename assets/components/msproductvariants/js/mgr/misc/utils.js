...
msProductVariants.utils.getMenu = function (actions, grid, selected) {
    var menu = [];
...
msProductVariants.utils.renderActions = function (value, props, row) {
    var res = [];
    ...
    for (var i in row.data.actions) {
        ...
        item = String.format(
            '<li class="{0}"><button class="msproductvariants-btn msproductvariants-btn-default {1}" action="{2}" title="{3}"></button></li>',
    ...
    return String.format(
        '<ul class="msproductvariants-row-actions">{0}</ul>',
        res.join('')
    );
};
